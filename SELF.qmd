...
format: pdf
---
September Update: picked new controls for modeling, produced and analyzed regression results and finished last regression needed.
...
data <- data.frame(
  PolicyPeriod = c("Before", "After", "Before", "After"),
  Gender = c("Male", "Male", "Female", "Female"),
  Score = c(523.2161, 525.7100, 546.4404, 551.6863)
)

pander(data, style = 'rmarkdown', caption = "Mean Scores by Policy Period and
       Gender")
...
```{r message=F,warning=F, echo=F,result='hide'}
#with label variable p034 "family gross income" without controls 
model2 <- lm(ZScore ~ Gender + PolicyPeriod + PolicyPeriod*Gender+
               as.factor(P034)+as.factor(P034)*PolicyPeriod, 
             data = analysis_data)
summary(model2)

##with controls
model3 <- lm(ZScore ~ Gender + PolicyPeriod + PolicyPeriod*Gender
             +as.factor(P034)+as.factor(P034)*PolicyPeriod 
             + P022 + as.factor(P037), data = analysis_data_new)
summary(model3)



library(pander)
regression_results <- summary(model3)$coefficients
results_table <- data.frame(
  Estimate = regression_results[, "Estimate"],
  `P-value` = regression_results[, "Pr(>|t|)"]
)
pander(results_table, caption = "Regression Results with Family Gross Income")
...
## with controls
model4 <- lm(ZScore ~ Gender + PolicyPeriod + PolicyPeriod*Gender+P022+
               P022*PolicyPeriod+as.factor(P034)+as.factor(P037),
             data = analysis_data)
summary(model4)

library(pander)
regression_results <- summary(model4)$coefficients
results_table <- data.frame(
  Estimate = regression_results[, "Estimate"],
  `P-value` = regression_results[, "Pr(>|t|)"]
)
pander(results_table, caption = "Regression Results with Hours Worked")
...
## interaction with family status

```{r message=F,  echo=F}
# P037 father education
model5 <- lm(ZScore ~ Gender + PolicyPeriod + PolicyPeriod*Gender+
               as.factor(P037)
             +as.factor(P037)*PolicyPeriod+as.factor(P034)+P022, 
             data = analysis_data)
summary(model5)
